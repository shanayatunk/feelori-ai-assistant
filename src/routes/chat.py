...
def get_ai_response(message, conversation_history=None):
    ...
    if product_catalog:
        for product in product_catalog.values():
            ...
            # Simple keyword matching in title and tags
            if any(k in title or k in tags for k in keywords):
                # Check price if a limit is specified
    ...
    if matches:
        return {
            ...
            "products": matches[:3] # Return up to 3 matches
        }
    ...
    # Default fallback if no other response is found
    return {
        "message": "I'm not sure I understood. Could you tell me more about what you're looking for, perhaps a product type or color?",
        "type": "general"
    }
